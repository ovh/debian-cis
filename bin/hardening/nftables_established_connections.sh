...
audit() {
    ...
    for allowed in $NFTABLES_INPUT_ALLOWED; do
        ...
        if nft list ruleset 2>/dev/null | awk '/hook input/,/}/' | grep -E "ip protocol $protocol ct state $state accept"; then
    ...
    for allowed in $NFTABLES_OUTPUT_ALLOWED; do
        ...
        if nft list ruleset 2>/dev/null | awk '/hook output/,/}/' | grep -E "ip protocol $protocol ct state $state accept"; then
