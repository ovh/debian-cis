...
test_audit() {
    ...
    echo "-a always,exit -F path=/usr/bin/chcon" >"$audit_rules_file"
    register_test retvalshouldbe 1
    # shellcheck disable=2154
    ...
    echo "-a always,exit -F path=/usr/bin/chcon -F perm=x -F auid>=${UID_MIN} -F auid!=unset -k perm_chng" >"$audit_rules_file"
    # Remove from runtime
    auditctl -W /usr/bin/chcon 2>/dev/null || true
