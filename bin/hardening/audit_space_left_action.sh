...
audit() {
    if ! $SUDO_CMD test -f "$AUDITD_CONF_FILE"; then
    ...
    space_left_action=$($SUDO_CMD grep -P -- '^\h*space_left_action\h*=\h*\S+' "$AUDITD_CONF_FILE" | awk -F= '{print $2}' | tr -d ' ' || true)
    ...
    admin_space_left_action=$($SUDO_CMD grep -P -- '^\h*admin_space_left_action\h*=\h*\S+' "$AUDITD_CONF_FILE" | awk -F= '{print $2}' | tr -d ' ' || true)
