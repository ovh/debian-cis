...
apply() {
    ...
    if [ "$INPUT_CHAIN_DROP" -ne 0 ]; then
        info "Please review your ip6tables default 'INPUT' policy. We wont apply it for you to avoid cutting the branch you are sitting on"
    fi

    if [ "$OUTPUT_CHAIN_DROP" -ne 0 ]; then
        info "Please review your ip6tables default 'OUTPUT' policy. We wont apply it for you to avoid cutting the branch you are sitting on"
    fi

    if [ "$FORWARD_CHAIN_DROP" -ne 0 ]; then
        info "Please review your ip6tables default 'FORWARD' policy. We wont apply it for you to avoid cutting the branch you are sitting on"
    fi
