...
test_audit() {
    ...
    local audit_rules_file="/etc/audit/rules.d/50-perm_chng.rules"
    # shellcheck disable=2154
    local audit_rules_backup="/tmp/${script}.50-perm_chng.rules.bak"
    ...
    echo "-a always,exit -F path=/usr/bin/setfacl" >"$audit_rules_file"
    register_test retvalshouldbe 1
    # shellcheck disable=2154
    ...
    echo "-a always,exit -F path=/usr/bin/setfacl -F perm=x -F auid>=${UID_MIN} -F auid!=unset -k perm_chng" >"$audit_rules_file"
    # Remove from runtime
    auditctl -W /usr/bin/setfacl 2>/dev/null || true
