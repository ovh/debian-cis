...
test_audit() {
    ...
    echo "-a always,exit -F path=/usr/bin/chacl" >"$audit_rules_file"
    register_test retvalshouldbe 1
    # shellcheck disable=2154
    ...
    echo "-a always,exit -F path=/usr/bin/chacl -F perm=x -F auid>=${UID_MIN} -F auid!=unset -k perm_chng" >"$audit_rules_file"
    # Remove from runtime
    auditctl -W /usr/bin/chacl 2>/dev/null || true
