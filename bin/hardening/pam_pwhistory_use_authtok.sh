...
audit() {
    ...
    if [ "$pam_configs_ok" -ne 0 ] || [ "$pam_file_ok" -ne 0 ]; then
        ...
        PAM_AUTHTOK_PRESENT=0
    fi
}

# This function will be called if the script status is on enabled mode
apply() {
    if [ "$PAM_AUTHTOK_PRESENT" -ne 0 ]; then
        ...
        # Add use_authtok to each pwhistory line, removing duplicates first
        for config_file in $config_files; do
            backup_file "$config_file"
            info "Processing $config_file"

            # First remove any existing use_authtok on pam_pwhistory.so lines to prevent duplicates
            sed -i '/pam_pwhistory\.so/ s/[[:space:]]*use_authtok//g' "$config_file"
            # Then add it once at the end of each pam_pwhistory.so line
            sed -i '/pam_pwhistory\.so/ s/$/ use_authtok/' "$config_file"
        done
