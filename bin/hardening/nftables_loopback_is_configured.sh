...
apply() {
    ...
    if [ "$NFTABLES_LOOPBACK" -ne 0 ]; then
        ...
        $SUDO_CMD nft add rule inet filter input iif lo accept
    fi

    if [ "$NFTABLES_LOOPBACK_DROP" -ne 0 ]; then
        info "adding nftables loopback drop configuration"
        $SUDO_CMD nft create rule inet filter input ip saddr 127.0.0.0/8 counter drop
    fi

    is_ipv6_enabled
    if [ "$FNRET" -eq 0 ] && [ "$NFTABLES_IPV6_LOOPBACK_DROP" -ne 0 ]; then
        info "adding nftables ipv6 loopback drop configuration"
        $SUDO_CMD nft add rule inet filter input ip6 saddr ::1 counter drop
    fi
}
