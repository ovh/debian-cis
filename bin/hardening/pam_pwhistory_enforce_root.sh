...
apply() {
    if [ "$OPTION_PRESENT" -ne 0 ]; then
        ...
        # Add enforce_for_root to each pwhistory line that doesn't have it
        for config_file in $config_files; do
            backup_file "$config_file"
            info "Processing $config_file"

            # Use sed to add enforce_for_root if not already present on pam_pwhistory.so lines
            sed -i '/pam_pwhistory\.so/ {
                /enforce_for_root/! s/$/ enforce_for_root/
            }' "$config_file"
        done
