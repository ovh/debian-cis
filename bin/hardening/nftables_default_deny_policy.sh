...
apply() {
    ...
    if [ "$INPUT_CHAIN_DROP" -ne 0 ]; then
        info "Please review your default 'input' policy. We wont apply it for you to avoid cutting the branch you are sitting on"
    fi

    if [ "$OUTPUT_CHAIN_DROP" -ne 0 ]; then
        info "Please review your default 'output' policy. We wont apply it for you to avoid cutting the branch you are sitting on"
    fi

    if [ "$FORWARD_CHAIN_DROP" -ne 0 ]; then
        info "Please review your default 'forward' policy. We wont apply it for you to avoid cutting the branch you are sitting on"
    fi
